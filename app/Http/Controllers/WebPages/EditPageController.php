    ...
    public function edit_page(Request $request)
    {
        ...
        switch($request->EditFooterBTN){
            case 'edit':
                ...
                $time_for_RPD->course = IntegerSanitizer::sanitize( $request->time_for_RPD_course);
                $time_for_RPD->semester = IntegerSanitizer::sanitize( $request->time_for_RPD_semester);
                $time_for_RPD->c_u = IntegerSanitizer::sanitize( $request->time_for_RPD_c_u);
                $time_for_RPD->lectures = IntegerSanitizer::sanitize( $request->time_for_RPD_lectures);
                $time_for_RPD->laboratory = IntegerSanitizer::sanitize( $request->time_for_RPD_laboratory);
                $time_for_RPD->laboratory_inter = IntegerSanitizer::sanitize( $request->time_for_RPD_laboratory_inter);
                $time_for_RPD->practice = IntegerSanitizer::sanitize( $request->time_for_RPD_practice);
                $time_for_RPD->practice_inter = IntegerSanitizer::sanitize( $request->time_for_RPD_practice_inter);
                $time_for_RPD->KSP = IntegerSanitizer::sanitize( $request->time_for_RPD_KSP);
                $time_for_RPD->OK = IntegerSanitizer::sanitize( $request->time_for_RPD_OK);
                $time_for_RPD->SP = IntegerSanitizer::sanitize( $request->time_for_RPD_SP);
                $time_for_RPD->control = IntegerSanitizer::sanitize( $request->time_for_RPD_control);
                $time_for_RPD->save();

                $total_akadem_hours = $userRpd->totalAkademHours;
                $total_akadem_hours->experts = IntegerSanitizer::sanitize( $request->total_akadem_hours_experts);
                $total_akadem_hours->to_plan = IntegerSanitizer::sanitize( $request->total_akadem_hours_to_plan);
                $total_akadem_hours->control_work = IntegerSanitizer::sanitize( $request->total_akadem_hours_control_work);
                $total_akadem_hours->SR = IntegerSanitizer::sanitize( $request->total_akadem_hours_SR);
                $total_akadem_hours->kontrol = IntegerSanitizer::sanitize( $request->total_akadem_hours_kontrol);
                $total_akadem_hours->inter_hour = IntegerSanitizer::sanitize( $request->total_akadem_hours_inter_hour);
                $total_akadem_hours->pr_podgot = IntegerSanitizer::sanitize( $request->total_akadem_hours_pr_podgot);
                $total_akadem_hours->save();

                $form_control = $userRpd->formControl;
                $form_control->examination = IntegerSanitizer::sanitize( $request->form_control_examination);
                $form_control->zachet = IntegerSanitizer::sanitize( $request->form_control_zachet);
                $form_control->zachet_with_grade = IntegerSanitizer::sanitize( $request->form_control_zachet_with_grade);
                $form_control->KP = IntegerSanitizer::sanitize( $request->form_control_KP);
                $form_control->KR = IntegerSanitizer::sanitize( $request->form_control_KR);
                $form_control->control = IntegerSanitizer::sanitize( $request->form_control_control);
                $form_control->RGR = IntegerSanitizer::sanitize( $request->form_control_RGR);
                $form_control->save();

                $credit_units = $userRpd->creditUnits;
                $credit_units->experts = IntegerSanitizer::sanitize( $request->credit_units_experts);
                $credit_units->fackts = IntegerSanitizer::sanitize( $request->credit_units_fackts);
                $credit_units->hour_in_c_u = IntegerSanitizer::sanitize( $request->credit_units_hour_in_c_u);
                $credit_units->save();
